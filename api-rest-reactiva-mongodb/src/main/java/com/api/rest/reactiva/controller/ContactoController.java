...
public class ContactoController {
    ...
    public Mono<ResponseEntity<Contacto>> actualizarContacto(@RequestBody Contacto contacto, @PathVariable String id) {
        return contactoRepository.findById(id)                          // Mono<Contacto>
                .flatMap(contactoExistente -> {                // flatMap: para hacer save (asíncrono)
                    contacto.setId(id);
                    return contactoRepository.save(contacto)            // Mono<Contacto>
                            .map(contactoActualizado ->        // map: transformación síncrona
                                    ResponseEntity.accepted().body(contactoActualizado));
                })
